 ...
 * @author jrcsilva
 */
public class AlturaNuvensMapReduce {
    public void runMapReduce(final String mesInicio, final String mesFim) throws Exception {
        System.out.println("<<<<<---------------- Executando MapReduce Altura Nuvens ---------------->>>>>");
        ...
        UserGroupInformation ugi = UserGroupInformation.createRemoteUser(Propriedades.USER_HDFS); 
            ...
            ugi.doAs(new PrivilegedExceptionAction<Void>() {
                public Void run() throws Exception {
                    ...
                    JobConf conf = new JobConf(AlturaNuvensMapReduce.class);
                    conf.setJobName("Altura Nuvens do Mes "+mesInicio+" ate o mes "+mesFim);
                    ...
                    conf.setMapperClass(AlturaNuvensMapper.class);
                    conf.setCombinerClass(AlturaNuvensReduce.class);
                    conf.setReducerClass(AlturaNuvensReduce.class);

                    conf.setInputFormat(TextInputFormat.class);
                    conf.setOutputFormat(TextOutputFormat.class);

                    FileInputFormat.setInputPaths(conf, new Path(Propriedades.INPUT_HDFS_AGROTIME));
                    FileOutputFormat.setOutputPath(conf, new Path(Propriedades.OUTPUT_ALTURANUVENS+mesInicio+mesFim));
