...
import com.agrotime.mapper.CoberturaNuvensMapper;
import com.agrotime.reduce.AlturaNuvensReduce;
import com.agrotime.reduce.CoberturaNuvensReduce;
 ...
 * @author jrcsilva
 */
public class CoberturaNuvensMapReduce {
    public void runMapReduce(final String mesInicio, final String mesFim) throws Exception {
        System.out.println("<<<<<---------------- Executando MapReduce Cobertura Nuvens ---------------->>>>>");
        ...
        UserGroupInformation ugi = UserGroupInformation.createRemoteUser(Propriedades.USER_HDFS); 
            ...
            ugi.doAs(new PrivilegedExceptionAction<Void>() {
                public Void run() throws Exception {
                    ...
                    JobConf conf = new JobConf(CoberturaNuvensMapReduce.class);
                    conf.setJobName("Cobertura Nuvens do Mes "+mesInicio+" ate o mes "+mesFim);
                    ...
                    conf.setMapperClass(CoberturaNuvensMapper.class);
                    conf.setCombinerClass(CoberturaNuvensReduce.class);
                    conf.setReducerClass(CoberturaNuvensReduce.class);

                    conf.setInputFormat(TextInputFormat.class);
                    conf.setOutputFormat(TextOutputFormat.class);

                    FileInputFormat.setInputPaths(conf, new Path(Propriedades.INPUT_HDFS_AGROTIME));
                    FileOutputFormat.setOutputPath(conf, new Path(Propriedades.OUTPUT_COBERTURANUVENS+mesInicio+mesFim));
