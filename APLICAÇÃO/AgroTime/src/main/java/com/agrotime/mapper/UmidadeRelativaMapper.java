 ...
 * @author jrcsilva
 */
public class UmidadeRelativaMapper extends MapReduceBase implements Mapper<LongWritable, Text, Text, FloatWritable> {
    ...
    public void map(LongWritable key, Text value, OutputCollector<Text, FloatWritable> output, Reporter reporter) throws IOException {
        ...
        if(Integer.parseInt(mes[1]) >= Integer.parseInt(mesInicio) && Integer.parseInt(mes[1]) <= Integer.parseInt(mesFim)) {
            output.collect(new Text(chave), new FloatWritable(Float.parseFloat(atributos[18])));
        }
    }
}
